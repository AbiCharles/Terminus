...
# Milestone 1 oracle: write the full earthquake-recurrence CLI to /app/quake.c
# (it persists in the shared container for milestones 2 and 3), compile it
# against libsqlite3, and exercise the `query` subcommand.
...
/app/quake query --db /app/catalog.db --region Cascadia --mag-min 3.0 --out /app/events.csv
echo "milestone 1: wrote $(wc -l < /app/events.csv) lines to /app/events.csv"
