...
# Milestone 2 oracle: train the discrete-time logistic-regression hazard model on
# the train person-period rows using the exact design matrix and hyperparameters
# from /app/spec.md §4, then serialize it with joblib.
cat > /app/train_model.py <<'PY'
"""Fit the discrete-time hazard model and serialize it."""
import os
...
print("trained discrete-time hazard model:", dict(zip(FEATURES, model.coef_[0].round(4))))
PY
