...
On `rollback_conditions[].metric`: use the lower-snake-case form of the full quantity each trigger monitors — the population-stability-index trigger uses `population_stability_index` (never an abbreviation like `psi`), the approval-rate trigger uses `weekly_approval_rate`, and the ROC-AUC floor uses `roc_auc`.
