...
# Milestone 1 oracle: write the full earthquake declustering CLI to /app/quake
# (an executable Ruby script using the sqlite3 gem) and exercise the `query` subcommand.
...
QUAKE_EOF
chmod +x /app/quake
/app/quake query --db /app/catalog.db --region Cascadia --out /app/query.csv
echo " wrote /app/query.csv"
