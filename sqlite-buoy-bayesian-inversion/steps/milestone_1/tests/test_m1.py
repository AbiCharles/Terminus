...
These tests verify that /app/protocol.json encodes the binding Edition 3 criteria
from the governing sections of /app/mission_notebook.md, and that none of the
superseded Edition 1/2 distractor values (e.g. the 2022/2023 epochs, the 0.1
temperature conversion, the 0.25/0.2 pressure conversion, the 'deployed' status,
or salinity inclusion) leaked into the protocol.
"""
...
class TestMilestone1:
    ...
    def test_epoch_and_time_unit(self, protocol):
        """Mission epoch and time unit must be the binding §6 values."""
    ...
    def test_inclusion_rules(self, protocol):
        """Active status and the included sensor types must match §7."""
    ...
    def test_unit_conversions(self, protocol):
        """Per-sensor unit conversions must be the binding §8 values, not superseded ones."""
        ...
        assert not math.isclose(temp, 0.1, abs_tol=1e-9)  # Edition 1 value
        assert not math.isclose(press, 0.25, abs_tol=1e-9)  # Edition 1 value
        assert not math.isclose(press, 0.2, abs_tol=1e-9)  # Edition 2 value

    def test_noise_model(self, protocol):
        """Per-sensor observation noise must match §9, not the superseded pressure value."""
        temp = protocol["sensors"]["temperature"]["noise_std"]
        press = protocol["sensors"]["pressure"]["noise_std"]
        assert math.isclose(temp, 0.05, abs_tol=1e-12)
        assert math.isclose(press, 0.2, abs_tol=1e-12)
        assert not math.isclose(press, 0.3, abs_tol=1e-9)  # Edition 2 value

    def test_priors(self, protocol):
        """Per-sensor Gaussian priors for offset and drift must match §10."""
        for stype in ("temperature", "pressure"):
            ...
            for param in ("offset", "drift"):
                assert math.isclose(got[param]["mean"], exp[param]["mean"], abs_tol=1e-12)
                assert math.isclose(got[param]["std"], exp[param]["std"], abs_tol=1e-12)
        # Superseded priors must not be used.
        assert not math.isclose(
            protocol["sensors"]["temperature"]["priors"]["offset"]["std"], 1.0, abs_tol=1e-9
        )
        assert not math.isclose(
            protocol["sensors"]["pressure"]["priors"]["drift"]["std"], 0.1, abs_tol=1e-9
        )

    def test_exclusion_intervals(self, protocol):
        """Exclusion windows must be exactly the binding §11 set (no superseded windows)."""
        ...
        assert got == expected
        # Superseded Edition 2 windows must not appear.
        assert ("ALL", "2023-07-01T00:00:00Z", "2023-07-05T00:00:00Z") not in got
        assert ("B002", "2023-08-10T00:00:00Z", "2023-08-20T00:00:00Z") not in got
